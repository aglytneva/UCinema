package com.example.ucinema.base
...
sealed class Either<out LEFT, out RIGHT> {
    ...
    inline fun <TYPE> map(transorm: (RIGHT) -> TYPE): Either<LEFT, TYPE> = when (this) {
        is Left -> Left(this.value)
        is Right -> Right(transorm(this.value))
    }

    inline fun <TYPE> mapLeft(transorm: (LEFT) -> TYPE): Either<TYPE, RIGHT> = when (this) {
        is Left -> Left(transorm(this.value))
